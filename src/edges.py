...
def load_image(width=900, parameters=None):
    if parameters['input_image']:
        img0 = imread(parameters['input_image'])
    else:
...
def calculate_edges(img, detection_type='HED', gauss=3, frame=True):
    # Find edges of image objects
    if detection_type == 'HED':
        img_edges = edges_hed(img, gauss=gauss)
    elif detection_type == 'DiBlasi':
        img_edges = edges_diblasi(img, gauss=gauss, details=4)
    else:
...
def edges_diblasi(img, gauss=5, details=1):
